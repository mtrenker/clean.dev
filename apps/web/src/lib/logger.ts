...
/** WS upgrade rejected – missing or malformed Authorization header. */
export const EVT_COCKPIT_WS_UPGRADE_REJECTED    = 'cockpit.ws.upgrade_rejected'    as const;
/** WS token lookup DB error. */
export const EVT_COCKPIT_WS_TOKEN_LOOKUP_ERROR  = 'cockpit.ws.token_lookup_error'  as const;
/** WS connection established (upgrade completed). */
export const EVT_COCKPIT_WS_CONNECTED           = 'cockpit.ws.connected'           as const;
/** WS connection closed cleanly or forcibly. */
export const EVT_COCKPIT_WS_DISCONNECTED        = 'cockpit.ws.disconnected'        as const;
/** WS socket error emitted by the `ws` library. */
export const EVT_COCKPIT_WS_SOCKET_ERROR        = 'cockpit.ws.socket_error'        as const;
/** WS message body was not valid JSON. */
export const EVT_COCKPIT_WS_INVALID_JSON        = 'cockpit.ws.invalid_json'        as const;
/** WS message failed Zod schema validation. */
export const EVT_COCKPIT_WS_VALIDATION_FAILED   = 'cockpit.ws.validation_failed'   as const;
/** WS client_hello deviceId did not match the authenticated token's device. */
export const EVT_COCKPIT_WS_PAIRING_MISMATCH    = 'cockpit.ws.pairing_mismatch'    as const;
/** WS session successfully created (client_hello accepted). */
export const EVT_COCKPIT_WS_PAIRED              = 'cockpit.ws.paired'              as const;
/** WS createSession DB error. */
export const EVT_COCKPIT_WS_SESSION_CREATE_ERROR = 'cockpit.ws.session_create_error' as const;
/** WS event batch persisted and acknowledged. */
export const EVT_COCKPIT_WS_BATCH_INGESTED      = 'cockpit.ws.batch_ingested'      as const;
/** WS event batch DB insert error. */
export const EVT_COCKPIT_WS_BATCH_INSERT_ERROR  = 'cockpit.ws.batch_insert_error'  as const;
/** WS session touchSession error. */
export const EVT_COCKPIT_WS_TOUCH_SESSION_ERROR = 'cockpit.ws.touch_session_error' as const;

/** Projector polling loop started. */
export const EVT_COCKPIT_PROJECTOR_STARTED      = 'cockpit.projector.started'      as const;
/** Projector polling loop stopped. */
export const EVT_COCKPIT_PROJECTOR_STOPPED      = 'cockpit.projector.stopped'      as const;
/** Projector cycle completed with ≥1 projections. */
export const EVT_COCKPIT_PROJECTOR_CYCLE        = 'cockpit.projector.cycle_complete' as const;
/** Projector failed to project a single project. */
export const EVT_COCKPIT_PROJECTOR_ERROR        = 'cockpit.projector.project_error' as const;
/** Projector successfully projected one project (debug-level). */
export const EVT_COCKPIT_PROJECTOR_PROJECTED    = 'cockpit.projector.projected'    as const;

/** Operator revoked a device. */
export const EVT_COCKPIT_DEVICE_REVOKED         = 'cockpit.device.revoked'         as const;
/** Operator pruned raw events. */
export const EVT_COCKPIT_PRUNE_COMPLETED        = 'cockpit.prune.completed'        as const;
