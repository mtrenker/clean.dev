    ...
    <SiteShell>
      ...
      <SiteSection>
        <SiteContainer narrow>
          <Card className="border-l-4 border-l-[var(--site-rust)] p-5 font-mono text-sm leading-7 text-[var(--site-ink-sec)]">
      ...
      <SiteSection border={false}>
        <SiteContainer narrow>
          {posts.length === 0 ? (
            <Card className="p-6 text-[var(--site-ink-sec)]">
              {intl.formatMessage({ id: 'blog.empty' })}
            </Card>
          ) : (
            <ol className="overflow-hidden rounded-[6px] border border-[var(--site-rule)] bg-[var(--site-panel)]">
              {posts.map((post) => (
                <li key={post.slug} className="border-t border-[var(--site-rule)] first:border-t-0">
                  <article className="p-6 md:p-8">
                    <time className="font-mono text-xs uppercase tracking-[0.16em] text-[var(--site-ink-mute)]" dateTime={post.frontmatter.date}>
                      {formatPostDate(post.frontmatter.date, locale)}
                    </time>
                    <h2 className="mt-4 text-3xl font-medium tracking-[-0.03em] text-[var(--site-ink)] md:text-4xl">
                      <Link href={`/blog/${post.slug}`} className="transition hover:text-[var(--site-rust)]">
                        {post.frontmatter.title}
                      </Link>
                    </h2>
                    <p className="mt-4 max-w-3xl leading-7 text-[var(--site-ink-sec)]">{post.frontmatter.description}</p>
                    ...
                    <Link
                      ...
                      className="mt-6 inline-flex font-mono text-xs font-semibold uppercase tracking-[0.14em] text-[var(--site-rust)] transition hover:text-[var(--site-ink)]"
                    >
