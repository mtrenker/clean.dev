...
const ProjectCard = ({ project, lang, hero = false, intl }: { project: Project; lang: Locale; hero?: boolean; intl: ReturnType<typeof createIntl> }) => (
  <Card as="article" className={`p-6 transition hover:border-[#d96e3f] ${hero ? 'lg:col-span-2' : ''}`}>
    <div className="flex flex-wrap items-start justify-between gap-4 border-b border-[#2c2924] pb-4">
      <div>
        <p className="font-mono text-[0.68rem] uppercase tracking-[0.16em] text-[#d96e3f]">{project.title[lang]}</p>
        <h3 className="mt-2 text-3xl font-medium tracking-[-0.03em] text-[#ede7d4]">{projectName(project, lang)}</h3>
      </div>
    ...
    <div className={hero ? 'mt-5 grid gap-6 md:grid-cols-[1.4fr_0.8fr]' : 'mt-5 space-y-5'}>
      <div>
        <p className="leading-7 text-[#c4bda9]">{project.description[lang]}</p>
        {project.highlights[lang].length > 0 && (
          <ul className="mt-5 space-y-2">
            {project.highlights[lang].slice(0, hero ? 5 : 3).map((highlight) => (
              <li key={highlight} className="grid grid-cols-[1rem_1fr] gap-2 text-sm leading-6 text-[#c4bda9]">
                <span className="font-mono text-[#7eaf6a]">+</span>
                <span>{highlight}</span>
              </li>
            ))}
          </ul>
      ...
      <aside className="space-y-5 border-t border-dashed border-[#2c2924] pt-5 md:border-l md:border-t-0 md:pl-6 md:pt-0">
        ...
        <div className="flex flex-wrap gap-1.5">
          {project.technologies.slice(0, hero ? 14 : 9).map((technology) => (
            <span key={technology} className="rounded-[2px] border border-[#2c2924] px-2 py-1 font-mono text-[0.65rem] text-[#8a8474]">
              {technology}
            </span>
          ))}
        </div>
...
const TimelineEntry = ({ project, lang }: { project: Project; lang: Locale }) => (
  <li className="grid gap-2 border-t border-[#2c2924] px-5 py-4 first:border-t-0 md:grid-cols-[8rem_14rem_13rem_1fr] md:items-start md:gap-4">
    <time className="font-mono text-xs tracking-[0.04em] text-[#8a8474]" dateTime={`${getYear(project.startDate)}`}>
      {formatYearRange(project, true)}
    </time>
    <span className="font-mono text-sm font-semibold text-[#ede7d4]">{projectName(project, lang)}</span>
    <span className="font-mono text-xs text-[#d96e3f]">{project.title[lang]}</span>
    <span className="text-sm leading-6 text-[#c4bda9]">{project.description[lang]}</span>
  </li>
);
    ...
    <SiteShell className="print:bg-white print:text-black">
      <PageHero
        ...
        aside={(
          <Card className="p-5 print:hidden">
            <Image src="/me.png" alt={msg(intl, 'work.img.alt')} width={280} height={320} className="h-64 w-full rounded-[4px] border border-[#2c2924] object-cover object-[50%_24%] grayscale-[10%]" priority />
            <div className="mt-5 border-t border-dashed border-[#2c2924] pt-4">
              <p className="text-2xl font-medium tracking-[-0.02em] text-[#ede7d4]">Martin Trenker</p>
              <p className="mt-2 font-mono text-xs leading-6 text-[#8a8474]">{msg(intl, 'work.subtitle')}</p>
            </div>
          </Card>
        )}
      />
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {(['cleanAgile', 'learner', 'automation'] as const).map((section, index) => (
              <Card key={section} className="p-6">
                <p className="font-mono text-3xl font-medium text-[#d96e3f]">0{index + 1}</p>
                <h2 className="mt-5 text-2xl font-medium tracking-[-0.02em] text-[#ede7d4]">{msg(intl, `work.section.${section}.heading`)}</h2>
                <p className="mt-3 leading-7 text-[#c4bda9]">{msg(intl, `work.section.${section}.p`)}</p>
              </Card>
            ))}
          </div>
        </SiteContainer>
      ...
      <SiteSection>
        <SiteContainer>
          ...
          <p className="mb-8 max-w-3xl leading-7 text-[#c4bda9]">{msg(intl, 'work.spotlight.lead')}</p>
      ...
      <SiteSection border={false} className="print:p-0">
        <SiteContainer className="print:max-w-none print:px-0">
          ...
          <ol className="overflow-hidden rounded-[6px] border border-[#2c2924] bg-[#1c1a16] print:border-black print:bg-white">
          ...
          <button
            ...
            className="mt-6 rounded-[3px] border border-[#2c2924] px-5 py-3 font-mono text-xs font-semibold uppercase tracking-[0.12em] text-[#ede7d4] transition hover:border-[#d96e3f] hover:text-[#d96e3f] print:hidden"
      ...
      <SiteSection border={false} className="bg-[#0e0d0a] md:py-20 print:hidden">
        <SiteContainer>
          <Card className="p-8 md:p-10">
            <h2 className="max-w-4xl text-3xl font-medium leading-tight tracking-[-0.03em] text-[#ede7d4] md:text-5xl">
              {msg(intl, 'work.cta.heading')}
            </h2>
            <p className="mt-5 max-w-3xl text-lg leading-8 text-[#c4bda9]">{msg(intl, 'work.cta.lead')}</p>
