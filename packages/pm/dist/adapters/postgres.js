    ...
    async createInvoice(data) {
        ...
        return this.db.transaction(async (tx) => {
            ...
            if (selectedEntries.length !== timeEntryIds.length) {
                ...
                throw new Error(`${missingIds.length} of ${timeEntryIds.length} time entries not found for client ${clientId}: [${missingIds.join(', ')}]`);
            }
            ...
            if (alreadyInvoiced.length > 0) {
                ...
                throw new Error(`${alreadyInvoiced.length} time entries are already invoiced: [${invoicedIds.join(', ')}]`);
            }
            // Calculate totals
            let subtotal = 0;
            ...
            const lineItems = await tx
                .select()
                .from(schema.invoiceLineItems)
                .where((0, drizzle_orm_1.eq)(schema.invoiceLineItems.invoiceId, invoice.id))
                .orderBy(schema.invoiceLineItems.position);
            return {
                ...invoice,
                client,
